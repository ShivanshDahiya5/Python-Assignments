...
def print_table(marks, grades):
    print("\n--- Report Card ---")
    # Using format strings for better alignment
    print(f"{'Name':<20}{'Marks':<8}{'Grade'}") 
    print("--------------------------------")
    for name in marks:
        print(f"{name:<20}{marks[name]:<8}{grades[name]}")
...
def main():
    while True:
        ...
        if choice in ("1", "2"):
            # The input() here effectively pauses execution until Enter is pressed
            input("Press enter to go back to menu.")

# CRITICAL FIX: The name should have double underscores
